...
public function index(Request $request)
{
    ...
    foreach ($projects as $project) {
        ...
        // ---------- GST ----------
        $totalGST = (float) OrderDetail::whereIn('order_id', function ($query) use ($companyId, $project) {
            $query->select('id')
                  ->from('orders')
                  ->where('company_id', $companyId)
                  ->where('project_id', $project->id);
        ...
        // ==================== ORDERS WITH PAYMENT LOGS (SAFE VERSION) ====================
        $orderDetails = Order::where('company_id', $companyId)
            ->where('project_id', $project->id)
            ->select([
                'id', 'customer_id', 'project_id', 'company_id', 'long', 'lat', 'profit',
            ...
            ->with(['income' => function ($q) {
                $q->select('id', 'order_id', 'received_amount', 'payment_type',
                          'receivers_bank', 'remark', 'received_by', 
                          'invoice_no', 'payment_date', 'invoice_date')  // Only existing columns
                  ->orderBy('payment_date', 'desc')   // Using payment_date instead of received_date
                  ->orderBy('id', 'desc');
            }])
            ->get()
            ->map(function ($order) {
                ...
                $totalReceived = $order->income->sum('received_amount');
                ...
                return [
                    ...
                    'sgst' => (float) ($order->sgst ?? 0),
                    'igst' => (float) ($order->igst ?? 0),
                    'gst_amount' => (float) $gstFromDetails,
                    'gst' => (float) ($order->gst ?? $gstFromDetails),

                    // Payment Logs (Safe)
                    'total_received_from_income' => (float) $totalReceived,
                    'payments' => $order->income->map(function ($inc) {
                        return [
                            'income_id'       => $inc->id,
                            'received_date'   => $inc->payment_date ?? $inc->invoice_date,
                            'received_amount' => (float) $inc->received_amount,
                            'payment_type'    => $inc->payment_type,
                            'bank'            => $inc->receivers_bank,
                            'remark'          => $inc->remark,
                            'received_by'     => $inc->received_by,
                            'invoice_no'      => $inc->invoice_no,
                        ];
                    })
                ];
            });
        ...
        $totalReceivedAll = $orderDetails->sum('total_received_from_income');
        ...
        $response[] = [
            ...
            'sales' => [
                ...
                'gst_amount'            => $totalGST,
                'final_amount_with_gst' => $finalAmountWithGST,
                'total_received'        => (float) $totalReceivedAll,
                'overall_pending'       => (float) ($finalAmountWithGST - $totalReceivedAll),
                'expense_amount'        => (float) $totalExpenses,
                'net_balance'           => (float) ($finalAmountWithGST - $totalExpenses),
            ],

            'order_summary' => [
                'total_amount_without_gst' => (float) ($finalAmountWithGST - $totalGST),
                'gst_amount'               => $totalGST,
                'final_amount_with_gst'    => $finalAmountWithGST,
                'paid_amount'              => $paidAmount,
                'pending_amount'           => $pendingAmount,
                'total_discount'           => $totalDiscount,
                'note'                     => 'Final Amount includes GST',
            ],
