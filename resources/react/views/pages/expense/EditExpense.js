...
import CIcon from '@coreui/icons-react';
import { cilTrash, cilCloudUpload, cilCloudDownload } from '@coreui/icons';
import { getAPICall, postFormData } from "../../../util/api";
import { useToast } from "../../common/toast/ToastContext";
import { useTranslation } from "react-i18next";
import i18n from "i18next";
...
// Helper: Resolve photo URL (handles both S3 and local paths)
...
const EditExpense = ({ visible, onClose, expense, onExpenseUpdated }) => {
  ...
  const openImagePreview = (photoUrl) => {
    if (!photoUrl) {
      showToast("warning", "Image URL not available");
      return;
    }
    const resolved = resolvePhotoUrl(photoUrl);
    console.log("Opening Image URL:", resolved); // Debug
    setSelectedPreviewImage(resolved);
  ...
  const handleDownloadPhoto = async (photoUrl) => {
    ...
    const fileName = photoUrl.split('/').pop().split('?')[0] || 'expense-photo.jpg';
  ...
  const handleSubmit = async (event) => {
    ...
    if (state.expense_id && state.price > 0 && state.qty > 0) {
      ...
      try {
        ...
        // Update remarks for existing photos
        existingPhotos.forEach(photo => {
          if (!photosToDelete.includes(photo.id)) {
            formData.append(`photo_remarks[${photo.id}]`, photo.remark || '');
          }
    ...
    <CModal visible={visible} onClose={handleClose} size="lg">
      ...
      <CModalBody>
        <CForm noValidate validated={validated} onSubmit={handleSubmit}>
          ...
          {(state.payment_type === "imps" || state.payment_type === "rtgs" || state.payment_type === "neft") && (
            <div className="row">
              <div className="col-sm-3 mb-3">
                <CFormLabel><b>Bank Name</b></CFormLabel>
                <CFormInput name="bank_name" value={state.bank_name || ""} onChange={handleChange} placeholder="Enter Bank Name" />
              </div>
              <div className="col-sm-3 mb-3">
                <CFormLabel><b>Account Number</b></CFormLabel>
                <CFormInput name="acc_number" value={state.acc_number || ""} onChange={handleChange} placeholder="Enter Account Number" />
              </div>
              <div className="col-sm-3 mb-3">
                <CFormLabel><b>IFSC</b></CFormLabel>
                <CFormInput name="ifsc" value={state.ifsc || ""} onChange={handleChange} placeholder="Enter IFSC" />
              </div>
              <div className="col-sm-3 mb-3">
                <CFormLabel><b>Transaction ID</b></CFormLabel>
                <CFormInput name="transaction_id" value={state.transaction_id || ""} onChange={handleChange} placeholder="Enter Transaction ID" />
              </div>
            </div>
          )}
          ...
          {state.payment_type === "cheque" && (
            <div className="row">
              <div className="col-sm-4 mb-3">
                <CFormLabel><b>Cheque Number</b></CFormLabel>
                <CFormInput name="transaction_id" value={state.transaction_id || ""} onChange={handleChange} placeholder="Enter Cheque Number" />
              </div>
              <div className="col-sm-4 mb-3">
                <CFormLabel><b>Bank Name</b></CFormLabel>
                <CFormInput name="bank_name" value={state.bank_name || ""} onChange={handleChange} placeholder="Enter Bank Name" />
          ...
          <div className="row">
            <div className="col-sm-4 mb-3">
              <CFormLabel><b>Aadhar Number</b></CFormLabel>
              <CFormInput name="aadhar" value={state.aadhar || ""} onChange={handleChange} placeholder="Enter Aadhar Number" />
            </div>
            <div className="col-sm-4 mb-3">
              <CFormLabel><b>PAN</b></CFormLabel>
              <CFormInput name="pan" value={state.pan || ""} onChange={handleChange} placeholder="Enter PAN" />
            </div>
            <div className="col-sm-4 mb-3">
              <CFormLabel><b>Pending Amount</b></CFormLabel>
              <CFormInput type="number" name="pending_amount" value={state.pending_amount || 0} onChange={handleChange} placeholder="Enter Pending Amount" />
            </div>
          </div>
          ...
          {existingPhotos.length > 0 && (
            <div className="mb-4">
              ...
              <div className="row g-3">
                {existingPhotos.map((photo) => {
                  const isMarkedForDeletion = photosToDelete.includes(photo.id);
                  const resolvedUrl = resolvePhotoUrl(photo.photo_url);
                      ...
                      <CCard className={isMarkedForDeletion ? 'border-danger' : ''}>
                        <CCardBody>
                          <div 
                            ...
                            onClick={() => !isMarkedForDeletion && openImagePreview(photo.photo_url)}
                          >
                            {photo.photo_type === 'pdf' ? (
                              <div className="text-center">
                                <CIcon icon={cilCloudUpload} size="4xl" />
                          ...
                          <div className="d-flex gap-2">
                            <CButton 
                              ...
                              size="sm" 
                              className="flex-fill"
                              onClick={() => handleDownloadPhoto(photo.photo_url)}
                            >
        ...
        <CModalBody className="text-center">
          {selectedPreviewImage && (
            <img
              ...
              onError={(e) => {
                e.target.onerror = null;
                e.target.src = 'https://via.placeholder.com/600x400?text=Failed+to+Load';
              }}
            />
          )}
        </CModalBody>
        <CModalFooter>
          <CButton color="primary" onClick={() => handleDownloadPhoto(selectedPreviewImage)}>
            <CIcon icon={cilCloudDownload} /> Download
          </CButton>
          <CButton color="secondary" onClick={() => setShowSingleImageModal(false)}>
            Close
          </CButton>
        </CModalFooter>
      </CModal>
