...
const IncomeTable = () => {
  ...
  const [projectTypes, setProjectTypes]   = useState([]);        // ← NEW
  ...
  const fetchIncomes = useCallback(async (page = 1, reset = false) => {
    ...
    try {
      ...
      if (projectTypeId)       url += `&project_type_id=${projectTypeId}`;
  ...
  useEffect(() => {
    ...
    fetchProjectTypes();
    fetchIncomes(1, true);
  }, [fetchProjects, projectId, fetchProjectTypes]);
...
useEffect(() => {
  ...
  setIncomes([]);          
  fetchIncomes(1, true);   
}, [projectId, projectTypeId, fetchIncomes]);
