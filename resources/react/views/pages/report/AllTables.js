...
function All_Tables({
      ...
      case '1':
        return (
          <>
            ...
            <CTableHeaderCell onClick={() => handleSort('projectType')} style={headerStyle}>
              {t('LABELS.project_type') || 'Type'}
              <span style={getSortIconStyle('projectType')}>{getSortIcon('projectType')}</span>
            </CTableHeaderCell>
  ...
  const renderTableRows = () => {
    ...
    if (data.length === 0) {
      const colSpan = selectedOption === '1' || selectedOption === '2' ? 4 : 6;
      return (
        <CTableRow>
      ...
      <CTableRow key={index} className="data-row">
        {selectedOption === '1' && (
          <>
            ...
            <CTableDataCell className="project-type-cell">
              {item.projectType || '-'}
            </CTableDataCell>
