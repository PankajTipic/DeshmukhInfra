...
const VendorLadger = () => {
  ...
  const fetchLedgerReport = async () => {
    ...
    try {
      ...
      const response = await getAPICall(`/api/vendor-ledger-report?${params.toString()}`);
      ...
      if (error.response?.status === 401) {
        ...
        alert('Failed to load vendor ledger report');
      }
    } finally {
      setLoading(false);
    }
  ...
  const fetchVendors = async () => {
    try {
      const response = await getAPICall('/api/typeTwoVendor');
      // Assume the response might be { status: true, data: [...] } or just an array
  ...
  const generatePdf = () => {
    ...
    const drawHeader = (vendor) => {
      ...
      doc.text("Vendor Ledger", pageWidth / 2, currentY + 12, { align: "center" });
    ...
    doc.save(`Vendor_Ledger_${today}.pdf`);
  };
  ...
  const exportExcel = () => {
    ...
    XLSXWriteFile(wb, 'Vendor_Ledger.xlsx');
  };
  ...
  return (
    <>
      <CCard className="mb-4">
        <CCardHeader>
          <h4 className="mb-0">Vendor Ledger Report</h4>
        </CCardHeader>
        <CCardBody>
          <CRow className="g-3 mb-4">
            <CCol md={3}>
              ...
              <Select
                ...
                options={vendors.map(v => ({ value: v.id, label: `${v.name} - ${v.mobile}` }))}
                value={filterVendor}
            ...
            <CCard className="mb-4 border-primary">
              ...
              <CCardBody>
                <CRow className="text-center">
                  ...
                  <CCol md={3}><h6>Total Purchase</h6><h4 className="text-danger">₹{formatCurrency(grandTotal.total_debit)}</h4></CCol>
                ...
                <CTable bordered hover responsive>
                  <CTableHead>
                    <CTableRow>
                      ...
                      <CTableHeaderCell className="text-end">Purchase (Dr)</CTableHeaderCell>
                      <CTableHeaderCell className="text-end">Paid (Cr)</CTableHeaderCell>
