...
const LedgerReport = () => {
  ...
  const exportExcel = () => {
    const ws = XLSXUtils.json_to_sheet(data.map(item => ({
      ...
      Customer: item.project.customer_name,
      'Work Order': item.summary.total_orders_amount,
      Received: item.summary.total_received_amount,
      Expense: item.summary.total_expense_amount,
      'Net Balance': item.summary.net_balance,
      Status: item.summary.balance_status,
    })));
    const wb = XLSXUtils.book_new();
    XLSXUtils.book_append_sheet(wb, ws, 'Ledger');
    XLSXWriteFile(wb, 'Regular_Project_Ledger.xlsx');
  };
