...
const RecordPaymentModal = ({ 
  ...
  const toMoney = (num) => Number(parseFloat(num || 0).toFixed(2))
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      const remainingAmount = toMoney(totalAmount - paidAmount)
  ...
  const entered = toMoney(formData.received_amount)

  if (entered > remainingAmount) {
    showAlert(`Entered amount cannot be greater than pending amount (₹${remainingAmount})`, 'danger')
    setFormData(prev => ({ ...prev, received_amount: '' }))
    return
  }

  // normalize value
  setFormData(prev => ({ ...prev, received_amount: entered.toFixed(2) }))
}
