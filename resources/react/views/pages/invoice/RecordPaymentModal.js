...
const RecordPaymentModal = ({
  ...
  useEffect(() => {
    ...
    const totalAmount = parseFloat(orderData.finalAmount ?? orderData.totalAmount ?? orderData.billing_amount ?? 0)
    const paidAmount = parseFloat(orderData.paidAmount ?? 0)
    const remainingAmount = totalAmount - paidAmount
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'received_amount') {
      ...
      // For edit: max allowed = current remaining (can only go up to what's still pending)
      // For new: max allowed = remaining
      const maxAllowed = isEdit ? remainingAmount : remainingAmount

      if (receivedAmount > maxAllowed) {
        showAlert(`Amount cannot exceed pending amount of ₹${maxAllowed.toLocaleString()}`, 'danger')
        return // prevent update — keeps old value
      }
    }
