...
//               ? projectTypes
//                   .filter((pt) => String(pt.id) === String(filters.project_type_id))
//                   .map((pt) => ({ value: pt.id, label: pt.name }))[0] || null
//               : null
//           }
//           onChange={(selected) =>
//             setFilters({ ...filters, project_type_id: selected ? selected.value : '' })
//           }
//           isClearable
//         />
//       </div>
...
//             filters.project_id
//               ? projects
//                   .filter((p) => String(p.id) === String(filters.project_id))
//                   .map((p) => ({
//                     value: p.id,
//                     label: `${p.project_name} - ${p.customer_name}`,
//                   }))[0] || null
...
//           }
//           onChange={(selected) =>
//             setFilters({ ...filters, project_id: selected ? selected.value : '' })
//           }
//           isClearable
//         />
//       </div>
...
// {order.is_subcontract !== 1 && order.project?.is_subcontract !== 1 && (
//   <CButton
//     color="dark"
//     size="sm"
...
import { useNavigate } from 'react-router-dom'
import { deleteAPICall, getAPICall, put } from '../../../util/api'
import { cilArrowRight, cilPencil, cilHistory, cilNotes, cilCloudDownload, cilChart, cilAperture, cilTrash } from '@coreui/icons'
import CIcon from '@coreui/icons-react'
import CombinedPaymentModal from './CombinedPaymentModal'
import SubcontractPaymentModal from './SubcontractPaymentModal'
import UpdateProjectModal from '../projects/UpdateProjectModal'
...
const OrderList = () => {
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      const workOrders = filteredOrders.filter(order => order.invoiceType === 2)
      setSelectedOrders(workOrders)
    } else {
      setSelectedOrders([])
    }
...
const filteredOrders = orders.filter(order => {
  const isSubcontractProject = order.project?.is_subcontract === 1
  const isSubcontractInvoice = order.invoiceType === 3

  if (activeTab === 'subcontract') {
    // Show invoiceType === 3 OR any invoice belonging to a subcontract project
    return isSubcontractInvoice || isSubcontractProject
  }
  // Normal tab: exclude subcontract invoices AND subcontract project orders
  return !isSubcontractInvoice && !isSubcontractProject
})
  ...
  // Count work orders for selection (only from filtered list)
  const workOrderCount = filteredOrders.filter(order => order.invoiceType === 2).length
  const allWorkOrdersSelected = workOrderCount > 0 && selectedOrders.length === workOrderCount

  // Tab counts
  const normalCount = orders.filter(o => o.invoiceType !== 3).length
  const subcontractCount = orders.filter(o => o.invoiceType === 3).length
          ...
          <CNav variant="tabs" className="mb-3">
            <CNavItem>
              ...
              >
                ...
                <CBadge color="secondary" className="ms-1">{normalCount}</CBadge>
              </CNavLink>
            </CNavItem>
            <CNavItem>
              <CNavLink
                active={activeTab === 'subcontract'}
                onClick={() => {
              ...
              >
                Subcontracts{' '}
                <CBadge color="info" className="ms-1">{subcontractCount}</CBadge>
              </CNavLink>
            </CNavItem>
          </CNav>
            ...
            <>
              <div className="table-responsive">
                <CTable bordered hover>
                  <CTableHead color="dark">
                    <CTableRow>
                      {/* Checkbox column only shown on normal tab */}
                      <CTableHeaderCell style={{ width: '50px' }}>
                        {activeTab === 'normal' ? (
                          <CFormCheck
                            checked={allWorkOrdersSelected}
                  ...
                  <CTableBody>
                    {filteredOrders.length > 0 ? (
                      ...
                      <CTableRow>
                        <CTableDataCell colSpan={10} className="text-center py-4">
                          <div className="text-muted">
                            <h6>No {activeTab === 'subcontract' ? 'subcontract' : ''} invoices found</h6>
                            <p>No records to display for this tab</p>
                          </div>
                        </CTableDataCell>
                      </CTableRow>
                    )}
