...
export const exportToPDF = async (data) => {
  ...
  const companyEmail = company.email_id || "shreyas.gijare.21@gmail.com";
  const companyAddress = `${company.land_mark || "Urali Kanchan"}, ${company.Dist || "Pune"}`.trim();
  const logoUrl = company.logo ? `${host}/${company.logo}` : null;
  ...
  // ─── Draw company header on EVERY page ───
  const drawCompanyHeader = () => {
    let y = margin;
    // Outer border (optional – remove if you don't like it)
    doc.setDrawColor(60);
    doc.setLineWidth(0.7);
    doc.rect(margin - 2, margin - 2, pageWidth - (margin - 2) * 2, pageHeight - (margin - 2) * 2);
    // Company name
    ...
    doc.setFontSize(15);
    doc.setTextColor(30, 30, 80);
    doc.text(companyName, margin + 8, y + 20);
    // Address, phone, email
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    doc.setTextColor(70);
    doc.text(companyAddress, margin + 8, y + 34);
    doc.text(`Phone: ${companyPhone}`, margin + 8, y + 46);
    doc.text(`Email: ${companyEmail}`, margin + 8, y + 58);
    // Logo (right side)
    const logoSize = 52;
    ...
    doc.setDrawColor(0);
    doc.line(margin + 4, y, pageWidth - margin - 4, y);
    return y + 16; // starting y for content below header
  };
  // ─── First page header + title ───
  let currentY = drawCompanyHeader();
  doc.setFontSize(13);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(0);
  doc.text("VENDOR PURCHASE & PAYMENT REPORT", pageWidth / 2, currentY, { align: "center" });
  currentY += 28;
  // Vendor info & summary
  const vendor = data.vendor_details || {};
  const vendorName = vendor.vendor_name || "—";
  ...
  const vendorStr = `Vendor: ${vendorName} (ID: ${vendorId}) • Mobile: ${vendorMobile} • ${vendorAddress}`;
  const vendorLines = doc.splitTextToSize(vendorStr, contentWidth - 20);
  doc.setFontSize(9.5);
  doc.text(vendorLines, margin + 10, currentY);
  currentY += vendorLines.length * 11 + 8;
  const summaryStr = `Total: ${formatAmount(grandTotal)} Paid: ${formatAmount(grandPaid)} Balance: ${formatAmount(grandBalance)}`;
  doc.setFont("helvetica", "bold");
  doc.text(summaryStr, pageWidth / 2, currentY, { align: "center" });
  currentY += 32;
  // ─── Projects loop ───
  for (const project of data.projects) {
    const projName = project.project_details?.project_name || "Unnamed Project";
    ...
    if (currentY > pageHeight - 260) {
      doc.addPage();
      currentY = drawCompanyHeader();
    }
    // Project title
    doc.setFillColor(225, 230, 245);
    doc.rect(margin + 4, currentY, contentWidth - 8, 26, "F");
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    doc.setTextColor(30);
    doc.text(`Project: ${projName} (ID: ${projId})`, margin + 12, currentY + 17);
    currentY += 34;
    // ─── Table setup ───
    const head = [["Material", "Qty", "Rate", "Total", "GST", "Date", "Paid", "Balance"]];
    const tableWidth = contentWidth - 8;
    const colWidths = [0.22, 0.06, 0.10, 0.10, 0.07, 0.12, 0.14, 0.19].map(p => tableWidth * p);
    const body = [];
    (project.purchases || []).forEach((pur, pIdx) => {
      const pd = pur.purchase_details || {};
      const pm = pur.payment_master || {};
      ...
      // Payment history or no transactions message
      if (logs.length > 0) {
        body.push([{
          content: "Payment History",
          colSpan: 8,
          styles: { fillColor: [255, 243, 224], textColor: [130, 60, 0], fontStyle: "bold", fontSize: 9, halign: "left", cellPadding: 5 }
        }]);
        logs.forEach(log => {
          body.push([
            log.payment_date || "—",
        ...
        body.push([{
          content: "No payment transactions recorded",
          colSpan: 8,
          styles: { fontStyle: "italic", textColor: [140, 140, 140], fontSize: 8, fillColor: [248, 248, 248], halign: "center", cellPadding: 6 }
        }]);
      }
      if (pIdx < (project.purchases?.length || 1) - 1) {
        body.push([{ content: "", colSpan: 8, styles: { minCellHeight: 14, fillColor: [255,255,255] } }]);
      }
    });
    // Project total row
    const pTotal = safeSum(project.purchases || [], "purchase_details.total");
    ...
    doc.autoTable({
      ...
      willDrawPage: (data) => {
        if (data.pageCount > 1) {
          const afterHeaderY = drawCompanyHeader();
          data.cursor.y = afterHeaderY;
        }
      },
      didDrawPage: (data) => {
        // Page number & generated date on every page
        doc.setFontSize(8);
        doc.setTextColor(90);
        doc.text(`Page ${doc.internal.getCurrentPageInfo().pageNumber}`, pageWidth - margin - 30, pageHeight - 18, { align: "right" });
        doc.text(`Generated: ${new Date().toLocaleString("en-IN")}`, margin + 8, pageHeight - 18);
      }
    });
...
export const exportDateWisePDF = async (ledgerData, appliedStartDate = "", appliedEndDate = "") => {
  if (!ledgerData?.ledger_entries?.length) {
    alert("No ledger data to export");
    return;
  }
  ...
  const companyAddress = addressParts.length ? addressParts.join(", ") : "—";
  const logoPath       = company.logo ? `${host}/img/${company.logo}` : null;

  let logoBase64 = null;
  if (logoPath) {
    try {
      logoBase64 = await loadImageToBase64(logoPath);
    } catch (err) {
      console.error('Failed to load logo:', err);
    }
  }
  ...
  doc.autoTable({
    ...
    willDrawPage: (data) => {
      if (data.pageCount > 1) {
        const afterHeaderY = drawHeaderAndBorder();
        data.cursor.y = afterHeaderY;
      }
    },
